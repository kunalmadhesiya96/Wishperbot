...
// Bot ka apna username cache karte hain (cold start pe ek baar fetch hota hai)
...
// Format: @botusername @target message
//         @botusername @target 5s message
//         @botusername 123456789 message
//         @botusername 123456789 10s message
const WHISPER_REGEX =
  /^@(\w+)\s+(@[a-zA-Z0-9_]{4,32}|\d{5,15})\s+(?:(\d{1,4})s\s+)?([\s\S]+)$/i;
...
bot.start(async (ctx) => {
  ...
  await ctx.reply(
    "👋 *Whisper Bot* mein swagat hai!\n\n" +
      ...
      `\`@${botUsername} @target_username tumhara secret message\`\n` +
      `\`@${botUsername} @target_username 5s tumhara secret message\`  (5 second baad message gayab)\n` +
      `\`@${botUsername} 123456789 message\`  (username ki jagah user ID bhi chalega)\n` +
      `\`@${botUsername} 123456789 10s message\`\n\n` +
...
bot.on("message", async (ctx) => {
  // Bot sirf Group Chat (group/supergroup) aur Private DM mein kaam karega.
  // Channel ya kisi aur chat type mein bilkul respond nahi karega.
  const allowedTypes = ["private", "group", "supergroup"];
  if (!allowedTypes.includes(ctx.chat.type)) return;
  ...
  const [, mentionedBotUsername, rawTarget, durationStr, whisperTextRaw] =
    match;

  // Pehla @mention bot ka apna username hona chahiye, tabhi aage badho
  const botUsername = await getBotUsername();
  if (mentionedBotUsername.toLowerCase() !== botUsername) return;
